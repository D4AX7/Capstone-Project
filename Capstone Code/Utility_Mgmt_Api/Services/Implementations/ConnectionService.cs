...
public class ConnectionService : IConnectionService
{
    ...
    public async Task<ApiResponse<ConnectionDto>> UpdateAsync(int id, UpdateConnectionDto dto)
    {
        ...
        if (
            ...
            && Enum.TryParse<ConnectionStatus>(dto.Status, true, out var status)
        )
        {
    ...
    public async Task<ApiResponse<bool>> DeleteAsync(int id)
    {
        ...
        // Check for pending/unpaid bills (OutstandingBalance > 0)
        ...
        if (pendingBills.Any())
        {
            ...
            return ApiResponse<bool>.ErrorResponse(
                $"Cannot delete connection. There are {pendingCount} pending bill(s) with a total outstanding balance of {totalOutstanding:C}. Please ensure all bills are paid before deleting this connection."
            );
        }
